...
-- ── Split navigation (VSCode-like Ctrl+\ then direction) ──
...
-- ── File Explorer (Ctrl+B like VSCode sidebar) ────────────
map("n", "<C-e>", function()
  ...
  if view.is_visible() then
    ...
    api.tree.open()
    -- Return focus to the previous window (the editor)
    vim.cmd("wincmd p")
  end
...
-- K, F2, C-. handled by lspsaga (see plugins/lsp.lua keys table)
...
-- ── Comment toggle (Ctrl+/ like VSCode) ───────────────────
-- Handled by Comment.nvim with gcc / gc in visual
...
-- ── Multi-cursor word (Ctrl+D like VSCode) ────────────────
-- Handled by vim-visual-multi plugin
